...
@Component
public class ChatPromptBuilder implements PromptBuilder<ChatPromptContext> {

    @Override
    public String build(ChatPromptContext context) {
        ...
        appendHistory(systemPromptBuilder, context);

        systemPromptBuilder.append("请基于以上对话历史，回答用户的新问题。");
        return systemPromptBuilder.toString();
    }

    private void appendHistory(StringBuilder systemPromptBuilder, ChatPromptContext context) {
        if (context.history().isEmpty()) {
            return;
        }

        systemPromptBuilder.append("--- 对话历史 ---\n");
        for (Map<String, String> msg : context.history()) {
